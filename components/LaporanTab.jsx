...
export default function LaporanTab() {
  ...
  // Master Data Laporan berdasarkan Periode (Dinamis & Interaktif)
  const reportData = {
    "Hari Ini": {
      sales: 154500,
      transactions: 2,
  ...
  const currentData = reportData[period] || reportData["Bulan Ini"];
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Filter Periode Pills (Sekarang Menggunakan Warna Kuning #FFC72C) */}
      <div className="grid grid-cols-3 gap-2 bg-amber-100/60 p-1.5 rounded-2xl border border-amber-200">
        {["Hari Ini", "7 Hari", "Bulan Ini"].map((p) => (
          <button
            ...
            onClick={() => {
              setPeriod(p);
              showNotification(`Menampilkan data periode: ${p}`);
            }}
      ...
      {/* Uang Masuk vs Hutang Progress Bar (Warna Biru Dihapus -> Menggunakan Kuning #FFC72C) */}
      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm space-y-3">
        ...
        <div className="w-full bg-slate-100 h-3.5 rounded-full overflow-hidden flex border border-slate-200">
          <div
            className="bg-[#FFC72C] h-full transition-all duration-500"
            style={{ width: `${currentData.ratioMoneyIn}%` }}
          ></div>
          <div
            className="bg-amber-600 h-full transition-all duration-500"
            style={{ width: `${currentData.ratioDebt}%` }}
          ></div>
        </div>
        ...
        <div className="flex justify-between items-center text-xs">
          <div>
            ...
            <p className="font-extrabold text-slate-800 text-sm mt-0.5">
              Rp {currentData.moneyIn.toLocaleString()}
            </p>
          </div>
      ...
      {/* Rincian Metode Pembayaran Uang Masuk */}
      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm space-y-3">
        ...
        <div className="flex justify-between items-center border-b pb-2 text-xs">
          ...
          <span className="font-extrabold text-slate-800">
            Rp {currentData.cash.toLocaleString()}
          </span>
        </div>
        ...
        <div className="flex justify-between items-center text-xs">
          ...
          <span className="font-extrabold text-slate-800">
            Rp {currentData.qris.toLocaleString()}
          </span>
        </div>
      </div>
    </div>
